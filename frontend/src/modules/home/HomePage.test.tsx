...
describe('HomePage redesign', () => {
  ...
  it('explains the web product and links both hero actions to templates', () => {
    ...
    expect(
      screen.getByLabelText('Demostración de una invitación web real'),
    ).toHaveTextContent('Emilia')
    expect(
      screen.queryByText('Cumpleaños de Alex', {
        selector: '.invitation-preview h2',
      }),
    ).not.toBeInTheDocument()
    expect(
      screen.getAllByRole('link', {
        name: /Crear mi invitación|Ver plantillas/,
      }),
    ).toHaveLength(3)
  ...
  it('uses real categories and featured template identifiers', () => {
    ...
    const featured = invitationTemplates.filter(
      (item) => item.isFeatured && item.isAvailable,
    )
      ...
      expect(
        templateLinks.some(
          (link) =>
            ...
            `/invitations/create?template=${template.id}`,
        ),
      ).toBe(true),
    )
  })
  ...
  it('shows actual and future capabilities honestly and includes the creator signature', () => {
    ...
    expect(
      screen.getByRole('link', { name: 'victorjaviervivas@gmail.com' }),
    ).toHaveAttribute('href', 'mailto:victorjaviervivas@gmail.com')
  })
  ...
  it('keeps content visible without IntersectionObserver and has no page-level horizontal overflow class', () => {
    ...
    expect(
      container.querySelectorAll('.home-reveal:not(.is-visible)'),
    ).toHaveLength(0)
    expect(container.querySelector('.redesigned-home')).toHaveClass(
      'redesigned-home',
    )
    expect(
      container.querySelector('.product-demo .wizard-preview'),
    ).toBeInTheDocument()
  })
  ...
  it('switches between light and dark themes and remembers the choice', async () => {
    ...
    expect(
      screen.getAllByRole('button', { name: 'Cambiar a tema claro' }).length,
    ).toBeGreaterThan(0)
  })
})
